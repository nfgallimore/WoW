--[[
	Action Bar Saver, Shadowed, boomboo
]]
...
function ABS:SaveProfile(name)
	...
	for actionID=1, MAX_ACTION_BUTTONS do
		...
		if (type and type ~= "flyout") then
			local spellName, subSpellName = GetSpellInfo(id)
			--print ("Type: ",type, "id: ", id, "subType: ",subType, "Spell ID: ", extraID, "SpellName: ", spellName)
		end
		...
		if( type and id and ( actionID < POSSESSION_START or actionID > POSSESSION_END ) ) then
			...
			if( type == "companion" ) then
				set[actionID] = string.format("%s|%s|%s|%s|%s|%s", type, id, "", name, subType or "", extraID or "")
			...
			-- Save a spell
			elseif( type == BOOKTYPE_SPELL and id > 0 ) then
				local spellName, subSpellName = GetSpellInfo(id)
				if( spellName ) then
					set[actionID] = string.format("%s|%d|%s|%s|%s|%s", type, id, "", spellName, subSpellName or "", extraID or "")
				end
			-- Save a macro
			...
			elseif(type == "flyout") then
				...
				set[actionID] = string.format("%s|%d|%s|%s", type, id, "", spellName)
			end
		end
	end
...
function ABS:RestoreProfile(name, overrideClass)
	...
	for book=1, MAX_SKILLLINE_TABS do
		...
		for i= offset + 1, offset + numSpells do
			local spell, subSpellName = GetSpellBookItemName(i, BOOKTYPE_SPELL)

			-- This way we restore the max rank of spells
			spellCache[spell] = i
			spellCache[string.lower(spell)] = i
...
function ABS:RestoreAction(i, type, actionID, binding, ...)
	...
	if( type == BOOKTYPE_SPELL ) then
		...
		--flyout spells are 'hidden', so we aren't checking if the spellbook contained it
